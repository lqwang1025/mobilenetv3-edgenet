...
def train(branchyNet, train_loader, batchsize=10000, num_epoch=20, main=False):
    ...
    for epoch in range(num_epoch):
        ...
        for step, data in enumerate(train_bar): 
            images, labels = data
            input_data = branchyNet.xp.asarray(images.numpy(), dtype=branchyNet.xp.float32)
            label_data = branchyNet.xp.asarray(labels.numpy(), dtype=branchyNet.xp.int32)
...
def test(branchyNet, test_loader, batchsize=10000, main=False):
...
def test_suite_B(branchyNet, test_loader, batchsize=10000, ps=np.linspace(0.1, 2.0, 10)):
    accs = []  # 准确率列表
    diffs = []  # 测试时间列表
    num_exits = []
    ...
    for p in ps:
        ...
        acc, diff, num_exit, _, max_entropy = test(branchyNet, test_loader, batchsize=batchsize)
        accs.append(acc)
        diffs.append(diff)
        num_exits.append(num_exit)
        max_entropies.append(max_entropy)
    return ps, np.array(accs), np.array(diffs) / float(len(test_loader)), num_exits, max_entropies


# 根据退出点阈值来获取代入单个退出点阈值的网络测试准确率、测试时间以及退出点样本数（带分支网络）
def screen_branchy(branchyNet, test_loader, base_ts, batchsize=1, enumerate_ts=True, verbose=False):
    # 生成关于所有退出点的阈值列表
    if enumerate_ts:
    ...
    # 对于阈值列表中的每一个阈值分别进行测试
    ts, accs, diffs, exits, max_entropies = test_suite_B(branchyNet, test_loader, batchsize=batchsize, ps=ts)
